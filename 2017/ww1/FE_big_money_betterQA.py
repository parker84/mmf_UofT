...
Note:
    ...
    - all the bugs Im aware of have now been caught
"""
...
class FE_df(object):
    ...
    def __init__(self, dfname, feat_names, primary_key='orderlineid',
        ...
        assert self.df.shape[0] == len(self.df[self.primary_key].unique()), \
                'number of columns != number of unique primary keys in the end'
    ...
    def get_feats_at_t(self, dft, col, t2):
        ...
        dfw = dft2.merge(dfa, on=col, how='left')
        print('dim dfw (combo with features and current dft): {}, number of unique values in the primary key col: {}'.format(
            dfw.shape, len(dfw[self.primary_key].unique())
        ))
        if dfw.shape[0] != len(dfw[self.primary_key].unique()):
            # lets make sure we have unique joining keys
            print('shape of dfa {}, num unique valus of {} in dfa: {}'.format(dfa.shape, col, len(dfa[col].unique())))
            print('shape of dft2 {}, num unique valus of {} in dft2: {}'.format(dft2.shape, col, len(dft2[col].unique())))
            print('shape of dft2 {}, num unique valus of {} in dft2: {}'.format(dft2.shape, self.primary_key, len(dft2[self.primary_key].unique())))
            print('head of dfa: {}, head of dft2: {}'.format(dfa.head(), dft2.head()))
        print(dfw.shape[0], len(dfw[self.primary_key].unique()))
        assert dfw.shape[0] == len(dfw[self.primary_key].unique()), \
                'extra rows after the left join feature {} onto dft at time: {}'.format(col, t2)
    ...
    def get_all_feats_at_t(self, dft, t2):
        ...
        for col in self.feat_names[1:]:
            ...
            assert df.shape[0] == len(df[self.primary_key].unique()), \
                    'extra columns after the left join features together at time: {}'.format(t2)
...
if __name__ == '__main__':
    ...
    fe.df.to_csv('features_from_python_gs2.csv')
