...
create table historical_features_per_curr_app as
  select target_table."SK_ID_CURR",
      ...
      --Note: for these there will only be one per, but we still need to pretend to aggregate
         sum(features_from_most_recent_prev_app.total_num_days_past_due_cc)  as total_num_days_past_due_cc_from_prev_app,
         sum(features_from_most_recent_prev_app.num_times_dpd)               as num_times_dpd_from_prev_app,
         sum(features_from_most_recent_prev_app.num_times_over_max_cc)       as num_times_over_max_cc_from_prev_app,
         sum(features_from_most_recent_prev_app.num_late_payments)           as num_late_payments_from_prev_app,
         sum(features_from_most_recent_prev_app.num_times_underpaid)         as num_times_underpaid_from_prev_app,
         sum(features_from_most_similar_prev_app.total_num_days_past_due_cc) as total_num_days_past_due_cc_from_prev_app_w_most_sim_cred,
         sum(features_from_most_similar_prev_app.num_times_dpd)              as num_times_dpd_from_prev_app_w_most_sim_cred,
         sum(features_from_most_similar_prev_app.num_times_over_max_cc)      as num_times_over_max_cc_from_prev_app_w_most_sim_cred,
         sum(features_from_most_similar_prev_app.num_late_payments)          as num_late_payments_from_prev_app_w_most_sim_cred,
         sum(features_from_most_similar_prev_app.num_times_underpaid)        as num_times_underpaid_from_prev_app_w_most_sim_cred
  from target_table as target_table
         ...
         left join (select prev_app.*
                    ...
                    where prev_app.days_termination is not null) as features_from_most_recent_prev_app
           on target_table."SK_ID_CURR" = features_from_most_recent_prev_app.sk_id_curr
         left join (select cred_diff.*
                    from (select prev_app.*,
                                 prev_app.AMT_CREDIT - target_table."AMT_CREDIT" as prev_app_cred_curr_app_cred_diff
                          from target_table as target_table
                    ...
                    where cred_diff.prev_app_cred_curr_app_cred_diff is not null) as features_from_most_similar_prev_app
           on target_table."SK_ID_CURR" = features_from_most_recent_prev_app.sk_id_curr
